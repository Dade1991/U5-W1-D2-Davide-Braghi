...
public class AppConfig {
    ...
    public Tavolo tavoloBean() {
        return new Tavolo(01, 4, StatoTavolo.LIBERO);
    }

    @Bean(name = "order")
    public Order ordineBean() {
        Tavolo tavolo = tavoloBean();

        List<Menu> elementiOrdinati = new ArrayList<>();
        elementiOrdinati.add(new Menu(pizzaSalamiBean().getName(), pizzaSalamiBean().getPrice(), List.of("senza cipolla")));
